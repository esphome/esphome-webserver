              ...
              ${
                group.value.map(
                  ...
                  >
                    <div>
                      ${
                        component.icon ? html`<iconify-icon
                            icon="${component.icon}"
                            height="24px"
                          ></iconify-icon>` :
                                         nothing}
                    ...
                    ${
                        component.domain === 'sensor' ? html`<esp-entity-chart
  ...
  _handleEntityRowClick(e: any) {
    if (e?.currentTarget?.domain === 'sensor') {
      ...
      e?.currentTarget?.classList.toggle(
          'expanded', !e.ctrlKey ? undefined : true);
    }
  }
  _handleTabHeaderDblClick(e: Event) {
    const doubleClickEvent =
        new CustomEvent('entity-tab-header-double-clicked', {
          bubbles: true,
          composed: true,
        });
...
type ActionRendererMethodKey =
    keyof Omit<ActionRenderer, ActionRendererNonCallable>;
...
class ActionRenderer {
  ...
  private _datetime(
    ...
    return html`
      ...
      this.actioner?.restAction(
          entity, `${action}?${opt}=${val.replace('T', ' ')}`);
    }}"
      />
    `;
  }
  ...
  private _select(
      entity: entityConfig, action: string, opt: string,
      options: string[]|number[], val: string|number|undefined) {
    return html`<select
      @change="${(e: Event) => {
      const val = (<HTMLTextAreaElement>e.target)?.value;
      this.actioner?.restAction(
          entity, `${action}?${opt}=${encodeURIComponent(val)}`);
    }}"
    >
  ...
  private _range(
      entity: entityConfig, action: string, opt: string, value: string|number,
      min?: string|undefined, max?: string|undefined, step = 1) {
    if (entity.mode == 1) {
  ...
  private _textinput(
      entity: entityConfig, action: string, opt: string, value: string|number,
      min: number|undefined, max: number|undefined, pattern: string|undefined) {
    return html`
      <input
  ...
  private _colorpicker(entity: entityConfig, action: string, value: any) {
    ...
    function rgb_to_str(rgbhex: string) {
      const rgb = rgbhex.match(/[0-9a-f]{2}/gi)?.map((x) => parseInt(x, 16)) ||
          [0, 0, 0];
  ...
  render_switch() {
    ...
    if (this.entity.assumed_state)
      ...
      return this._switch(this.entity);
  }
  ...
  render_fan() {
    ...
    return [
      ...
      this.entity.speed_count ?
          this._range(
              this.entity, `turn_${this.entity.state.toLowerCase()}`,
              'speed_level',
              this.entity.speed_level ? this.entity.speed_level : 0, 0,
              this.entity.speed_count, 1) :
          '',
    ];
  }
  ...
  render_light() {
    ...
    return [
      ...
      width: 100%;">
        ${this._switch(this.entity)}
        ${
          this.entity.brightness ? this._range(
                                       this.entity, 'turn_on', 'brightness',
                                       this.entity.brightness, 0, 255, 1) :
                                   ''}
        ${
          this.entity.color_mode === 'rgb' ||
                  this.entity.color_mode === 'rgbw' ?
              this._colorpicker(this.entity, 'turn_on', this.entity?.color) :
              ''}
        ${
          this.entity.effects?.filter((v) => v != 'None').length ?
              this._select(
                  this.entity, 'turn_on', 'effect', this.entity.effects || [],
                  this.entity.effect) :
              ''}
      </div> `,
    ];
  ...
  render_select() {
    if (!this.entity) return;
    return this._select(
        this.entity, 'set', 'option', this.entity.option || [],
        this.entity.value);
  }
  ...
  render_number() {
    ...
    return html`
      ${
        this._range(
            this.entity, 'set', 'value', this.entity.value,
            this.entity.min_value, this.entity.max_value, this.entity.step)}
      ${this.entity.uom}
    `;
  }
  ...
  render_text() {
    ...
    return this._textinput(
        this.entity, 'set', 'value', this.entity.value, this.entity.min_length,
        this.entity.max_length, this.entity.pattern);
  }
  ...
  render_climate() {
    ...
    let current_temp =
        html`<div class="climate-row" style="padding-bottom: 10px";>
                              <label>Current:&nbsp;${
            this.entity.current_temperature} °C</label>
                            </div>`;

    if (this.entity.target_temperature_low !== undefined &&
      ...
      target_temp = html`
        <div class="climate-row">
          ...
          this._range(
              ...
              this.entity.target_temperature_low, this.entity.min_temp,
              this.entity.max_temp, this.entity.step)}
        </div>
        <div class="climate-row">
          <label>Target High:&nbsp;</label>
          ${
          this._range(
              this.entity, 'set', 'target_temperature_high',
              this.entity.target_temperature_high, this.entity.min_temp,
              this.entity.max_temp, this.entity.step)}
        </div>`;
    } else {
      target_temp = html`
        <div class="climate-row">
          ...
          this._range(
              ...
              this.entity.target_temperature!!, this.entity.min_temp,
              this.entity.max_temp, this.entity.step)}
        </div>`;
    }
    let modes = html``;
    if ((this.entity.modes ? this.entity.modes.length : 0) > 0) {
      modes = html`
        <div class="climate-row">
          ...
          ${
          this._select(
              this.entity, 'set', 'mode', this.entity.modes || [],
              this.entity.mode || '')}
        </div>`;
    }
    return html`
